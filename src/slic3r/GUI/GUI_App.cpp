...
void GUI_App::post_init()
{
    ...
    if (!this->init_params->input_files.empty()) {
        ...
        if (this->init_params->input_files.size() == 1 &&
            boost::starts_with(this->init_params->input_files.front(), "momentslicer://open")) {
            auto input_str_arr = split_str(this->init_params->input_files.front(), "momentslicer://open/?file=");
    ...
    if (data_dir().empty()) {
        ...
        if (!boost::filesystem::exists(data_dir_path)){
            ...
            if(boost::filesystem::exists(older_data_dir)){
                ...
                boost::filesystem::rename(data_dir_path / "BambuStudio.conf", data_dir_path / "MomentSlicer.conf", ec);
                boost::filesystem::rename(data_dir_path / "BambuStudio.conf.bak", data_dir_path / "MomentSlicer.conf.bak", ec);
            }
            else
	...
	if (m_app_conf_exists) {
        ...
        if (!error.empty()) {
            ...
            throw Slic3r::RuntimeError(
                _u8L("MomentSlicer configuration file may be corrupted and is not abled to be parsed."
...
bool GUI_App::load_language(wxString language, bool initial)
{
    if (initial) {
        ...
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(trace) << boost::format("language provided by MomentSlicer.conf: %1%") % language;
        else {
...
void GUI_App::MacOpenURL(const wxString& url)
{
    ...
    if (!url.empty() && boost::starts_with(url, "momentsliceropen://")) {
        auto input_str_arr = split_str(url.ToStdString(), "momentsliceropen://");
