...
void GCodeViewer::render_legend(float &legend_height, int canvas_width, int canvas_height, int right_margin)
{
        ...
        std::function<void()> callback = nullptr)
    {
        ...
        if (callback) {
            ...
            if (b_menu_item)
                callback();
            if (checkbox) {
                ImGui::SameLine(ImGui::GetWindowWidth() - ImGui::CalcTextSize(_u8L("Display").c_str()).x / 2 - ImGui::GetFrameHeight() / 2 - 2 * window_padding);                ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0.0, 0.0));
                ImGui::PushStyleColor(ImGuiCol_CheckMark, ImVec4(0.59f, 0.00f, 0.00f, 1.00f));
    ...
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.59f, 0.00f, 0.00f, 1.00f));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.59f, 0.00f, 0.00f, 0.78f));
