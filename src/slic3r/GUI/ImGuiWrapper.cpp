...
const ImVec4 ImGuiWrapper::COL_RED_LIGHT       = ImVec4(0.98f, 0.93f, 0.93f, 1.0f);
...
void ImGuiWrapper::push_combo_style(const float scale)
{
    if (m_is_dark_mode) {
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 1.0f * scale);
        ImGui::PushStyleColor(ImGuiCol_PopupBg, ImGuiWrapper::COL_WINDOW_BG_DARK);
        ImGui::PushStyleColor(ImGuiCol_BorderActive, COL_MOMENT);
        ImGui::PushStyleColor(ImGuiCol_HeaderHovered, to_ImVec4(to_rgba(ColorRGB::MOMENT(), 0.5f)));
        ImGui::PushStyleColor(ImGuiCol_HeaderActive, COL_MOMENT);
        ImGui::PushStyleColor(ImGuiCol_Header, COL_MOMENT);
...
{
    if (m_is_dark_mode) {
        ImGui::PushStyleColor(ImGuiCol_CheckMark, to_ImVec4(decode_color_to_float_array("#670000"))); // MOMENT use orca color for radio buttons
    } else {
