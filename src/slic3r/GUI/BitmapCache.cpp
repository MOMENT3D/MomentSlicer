    ...
    const bool grayscale/* = false*/, const bool dark_mode/* = false*/, const std::string& new_color /*= ""*/, const float scale_in_center/* = 0*/)
{
    ...
    if (dark_mode) {
        ...
        replaces["\"#960000\""] = "\"#670000\"";
        replaces["\"#F1F1F1\""] = "\"#36363B\"";
        replaces["#DBDBDB"] = "#4A4A51"; // MOMENT border color
        replaces["#F0F0F1"] = "#333337"; // MOMENT disabled background color
        replaces["#262E30"] = "#EFEFF0"; // MOMENT
    } else {
        replaces["#949494"] = "#7C8282"; // MOMENT replace icon line color for light theme
    }

    if (strstr(bitmap_name.c_str(), "toggle_on") != NULL && dark_mode) // MOMENT only replace color of toggle button
        replaces["#960000"] = "#670000";

    if (!new_color.empty())
        replaces["\"#960000\""] = "\"" + new_color + "\"";
...
bool BitmapCache::load_from_svg_file_change_color(const std::string &filename, unsigned width, unsigned height, ImTextureID &texture_id, const char *hexColor)
{
    ...
    unsigned int green_color = 0xFF889600; // #960000
