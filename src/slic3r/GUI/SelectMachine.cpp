    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(103, 0, 0), StateColor::Pressed), std::pair<wxColour, int>(wxColour(182, 76, 76), StateColor::Hovered),
...
bool SelectMachineDialog::is_show_timelapse()
{
    ...
    if (gcode_file.fail()) {
        ...
        while (gcode_file >> line) {
            ...
            if (line == "BambuStudio" || line == "MomentSlicer")
                is_version = true;
        }
    }
    gcode_file.close();
