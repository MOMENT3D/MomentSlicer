        ...
        if (mode == PrintEstimatedStatistics::ETimeMode::Normal) {
            ...
            if (EPSILON < block.trapezoid.accelerate_until && block.trapezoid.accelerate_until < block.distance - EPSILON) {
                ...
                if ((position - prev_move.position).norm() > EPSILON &&
                    ...
                    const float feedrate = curr_move.feedrate; //  MOMENT: set feedrate to the gcode feed rate to prevent visualiser from
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: MomentSlicer is also "bambu". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-bambu
    { EProducer::MomentSlicer, SLIC3R_APP_NAME },
    { EProducer::MomentSlicer, "generated by MomentSlicer" },
    { EProducer::MomentSlicer, "generated by BambuStudio" },
    { EProducer::MomentSlicer, "BambuStudio" }
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by MomentSlicer,
        // extract the config from it
        if (m_producer == EProducer::MomentSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from MomentSlicer's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::MomentSlicer) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::MomentSlicer: { return process_bambuslicer_tags(comment); }
...
// BambuStudio used extruder_id*2+mode to support per-nozzle limits, but MomentSlicer
