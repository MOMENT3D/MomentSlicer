#!/usr/bin/env bash
#
# Build MomentSlicer Flatpak locally using Docker with the same container image
...
echo "=== MomentSlicer Flatpak Build ==="
...
# ---------- prepare manifest ----------
MANIFEST_SRC="scripts/flatpak/com.momentslicer.MomentSlicer.yml"
MANIFEST_DOCKER="scripts/flatpak/com.momentslicer.MomentSlicer.docker.yml"
...
flatpak-builder $FORCE_CLEAN_FLAG \
    ...
    --repo=flatpak-repo \
    flatpak-build \
    scripts/flatpak/com.momentslicer.MomentSlicer.docker.yml
builder_end=$(date +%s)
