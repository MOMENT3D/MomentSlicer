...
# MomentSlicer Flatpak Build Script
# This script builds and packages MomentSlicer as a Flatpak package locally
...
show_help() {
    ...
    echo ""
    echo "Build MomentSlicer as a Flatpak package"
    echo ""
    echo "Options:"
...
echo -e "${BLUE}MomentSlicer Flatpak Build Script${NC}"
echo -e "${BLUE}================================${NC}"
...
if [[ -f "version.inc" ]]; then
    VER_PURE=$(grep 'set(MOMENT3D_VERSION' version.inc | cut -d '"' -f2)
    VER="V$VER_PURE"
    DATE=$(date +'%Y%m%d')
...
if [[ ! -f "./scripts/flatpak/com.momentslicer.MomentSlicer.yml" ]]; then
    echo -e "${RED}Error: Flatpak manifest not found at scripts/flatpak/com.momentslicer.MomentSlicer.yml${NC}"
    exit 1
fi
...
BUNDLE_NAME="MomentSlicer-Linux-flatpak_${VER}_${ARCH}.flatpak"
...
MANIFEST="scripts/flatpak/com.momentslicer.MomentSlicer.yml"
if [[ "$NO_DEBUGINFO" == true ]]; then
    MANIFEST="scripts/flatpak/com.momentslicer.MomentSlicer.no-debug.yml"
    sed '/^build-options:/a\  no-debuginfo: true\n  strip: true' \
        scripts/flatpak/com.momentslicer.MomentSlicer.yml > "$MANIFEST"
...
if ! flatpak-builder \
    ...
    rm -f "scripts/flatpak/com.momentslicer.MomentSlicer.no-debug.yml"
    exit 1
fi
...
echo ""
echo -e "${BLUE}To run MomentSlicer:${NC}"
echo -e "flatpak run com.momentslicer.MomentSlicer"
echo ""
echo -e "${BLUE}To uninstall:${NC}"
echo -e "flatpak uninstall --user com.momentslicer.MomentSlicer"
echo ""
if [[ "$FORCE_CLEAN" != true ]]; then
